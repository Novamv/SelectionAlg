...
def get_parser():
    ...
    parser.add_argument("--time-window", default="2.0", type = float, help="Time window of events in seconds")
...
if __name__ == "__main__":
    ...
    bufMgr.property("TimeWindow").set([0.0-args.time_window, args.time_window]) # i.e [-1s; 1s]
    ...
    #EventTagSvc
    import EventTagSvc
    eventtagsvc = task.createSvc("EventTagSvc")

    # Create alg
    # import SelectionAlg
    Sniper.loadDll("libSelectionAlg.so")
    alg = task.createAlg("SelectionAlg/alg_example")
    ...
    alg.property("recEDMPath").set(args.recEDMPath)
    ...
    neutronclassifier.property("recEDMPath").set(args.recEDMPath)
    neutronclassifier.property("FiducialVolume").set(args.fiducial_volume)

    import IBDSelectionTool
    ...
    basename = ""
    if(len(inputs) == 1):
        basename = inputs[0].rsplit("/", 1)[-1] 
    print(basename)

    alg.property("Filename").set(basename)
